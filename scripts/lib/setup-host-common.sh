...
setup_create_host() {
  ...
  if [[ -e "$target_directory" ]]; then
    [[ -f "$target_directory/host.nix" ]] \
      || setup_die "host already exists and was not generated by this setup script: $target_directory"
    if [[ -n "$hardware_source" && ! -f "$target_directory/hardware.nix" ]]; then
      install -m 0644 "$hardware_source" "$target_directory/hardware.nix" \
        || setup_die "failed to copy $hardware_source"
    fi
    setup_log "reusing existing generated host hosts/$host_name"
    return 0
  fi
...
setup_enable_flakes() {
  ...
  local nix_config_directory="$home_directory/.config/nix"
  local nix_config_file="$nix_config_directory/nix.conf"
  local required_features='extra-experimental-features = nix-command flakes'

  install -d -m 0755 "$nix_config_directory"
  touch "$nix_config_file"

  if ! awk '
    /^[[:space:]]*(extra-)?experimental-features[[:space:]]*=/ \
      && /nix-command/ && /flakes/ { found = 1 }
    END { exit !found }
  ' "$nix_config_file"; then
    printf '\n# Added by dotnix setup.\n%s\n' "$required_features" >> "$nix_config_file"
    setup_log "enabled nix-command and flakes in $nix_config_file"
  else
    setup_log "nix-command and flakes are already enabled in $nix_config_file"
  fi
