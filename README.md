...
セットアップ時には `~/.config/nix/nix.conf` へ `nix-command` と `flakes` を有効にする設定を追加し、初回実行でも各Nixコマンドへ同じ機能を明示します。rebuildやHome Manager適用の途中で失敗した場合は、同じホスト名でスクリプトを再実行すると生成済みのホスト構成を再利用して処理を続行します。
