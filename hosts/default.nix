...
let
  ...
  # setup scripts place the metadata for newly generated hosts next to the
  # host configuration. Existing hand-written hosts remain in staticHosts.
  directoryEntries = builtins.readDir ./.;
  generatedHostNames = builtins.filter (
    name: directoryEntries.${name} == "directory" && builtins.pathExists (./. + "/${name}/host.nix")
  ) (builtins.attrNames directoryEntries);
  generatedHosts = builtins.listToAttrs (
    map (name: {
      inherit name;
      value = import (./. + "/${name}/host.nix") { inherit inputs; };
    }) generatedHostNames
  );
in
staticHosts // generatedHosts
